...
public class TestAssertionAnalysis {
    ...
    public void returnsFrequencyOverMultipleAgents() {
        ...
        context.checking(new Expectations() {{
            exactly(20).of(pool).pick();
            will(returnValue(object));
            exactly(8).of(agent1).assertion(observation);
            will(returnValue(assertion1));
            exactly(2).of(agent1).assertion(observation);
            will(returnValue(assertion2));
            exactly(6).of(agent2).assertion(observation);
            will(returnValue(assertion1));
            exactly(4).of(agent2).assertion(observation);
            will(returnValue(assertion2));
        }});
